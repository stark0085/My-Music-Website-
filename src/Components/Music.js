...
export default function Music() {
  ...
  useEffect(() => {
    window.matchMedia("(max-width: 1050px)").addEventListener('change', () => {
      document.querySelector('.container').style.paddingTop = '8vw'
    })
    window.matchMedia("(max-width: 900px)").addEventListener('change', () => {
      document.querySelector('.container').style.paddingTop = '10vw'
    })
  }, []);


  return (
    <div className="container py-2" style={{ backgroundColor: 'black', paddingTop: '6vw' }}>
      <div className="row my-3 mx-2 px-3 flex flex-wrap">
        {items.map((element) => {
          return <div className="col my-2" style={{ width: '21.3vw' }} key={element.data.uri}>
            <MusicCard title={element.data.name} imgUrl={element.data.albumOfTrack.coverArt.sources[0].url} url={element.data.uri} artist={element.data.artists.items[0].profile.name} />
          </div>
        })}
      </div>
    </div>
